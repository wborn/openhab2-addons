 ...
 */
@NonNullByDefault
public class SDMDiscoveryService extends AbstractDiscoveryService {

    private final Logger logger = LoggerFactory.getLogger(SDMDiscoveryService.class);
    private SDMAccountHandler accountHandler;
    private @Nullable Future<?> discoveryJob;

    public SDMDiscoveryService(SDMAccountHandler accountHandler) {
    ...
    private String getDeviceLabel(SDMDevice device, SDMDeviceType type) {
        ...
        return displayName.isEmpty() ? String.format("Nest %s", typeLabel)
